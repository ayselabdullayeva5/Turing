...
let country11070 = null;
for (let i = 0; i < data.length; i++) {
  if (data[i].id === 11070) {
    country11070 = data[i].shipAddress.country;
    break;
  }
}
console.log(country11070);

// 2.Ölkəsi Germany olan obyekti tapın və consoledə yazdırın.

let germanyOrder = null;
for (let i = 0; i < data.length; i++) {
...
for (let i = 0; i < data.length; i++) {
  if (data[i].id === 10550) {
    salesCount = data[i].details.length;
    break;
  }
}
...
// 5.ID-i 10310 olan obyektin toplam qazancını tapın consoleda yazdırın (discount daxil)
...
for (let i = 0; i < data.length; i++) {
  if (data[i].id === 10310) {
    for (let j = 0; j < data[i].details.length; j++) {
      const detail = data[i].details[j];
      totalProfit += detail.unitPrice * detail.quantity * (1 - detail.discount);
    }
    break;
  }
}
